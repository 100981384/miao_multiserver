...
class systemMember extends Controller{
    ...
    public static function newMemberData(){
        return new FileCache("data/system/system_member.php");
    }
	//保证只加载一次文件
	public static function loadData(){
		if(is_null(self::$staticSql)){
			self::$staticSql = self::newMemberData();
		}
	...
	public function getAllUser($groupID='0') {
		$result = self::userAtGroup(0);
		foreach($result as $key=>&$val){
			unset($val['password']);
		}
		return $result;
	 ...
	 */
	public function add2($user = false){
