...
class explorer extends Controller{
	...
	public function mkdir(){
		...
		//miaomiao -s
		$patt = "{.+MiaoMcServer[^/]+$}";//正则禁止携带MiaoMcServer
	    if(preg_match($patt,$new)){
	...
	public function pathRname(){
		...
		//miaomiao s 
		$patt = "{.+MiaoMcServer[^/]+$}";
		if (file_exists_case($rnameTo)||preg_match($patt,$rnameTo)) {
	...
	public function pathDelete(){
		...
		foreach ($list as $val) {
			...
			if( $removeToRecycle !="1"  ||
				...
				$patt = "{.+MiaoMcServer[^/]+/$}";
				$this->udpGet($pathThis);
				if(preg_match($patt,$pathThis)){
				    $this->udpGet("yes");
				    show_json(LNG('no_permission_ext'),false);
				}
				else if(move_path($pathThis,$filename,'',$this->config['user']['fileRepeat'])) {
					$success++;
				}else{
					$error++;
				}
